...
const ECOMMERCE_STORE_ID = "store_01M0Q9D8YGK07157T3RRBMZR50";
...
const extractVariants = (variants) => {
  return (variants || []).map((v) => {
    ...
    return {
      ...
      booking_event: v?.booking_event || null,
    };
  });
};
 ...
 * @param {string|string[]} [params.type_ids] - Product type id(s) to include (optional)
 * @param {string|string[]} [params.collection_ids] - Collection id(s) to include (optional)
 * @param {string} [params.sort] - Raw query string (e.g. "order=ASC&sort_by=price") (optional)
 *
...
export async function getProducts({
  ...
  // `sort` is a raw query string (e.g. "order=ASC&sort_by=price"), matching StoreApi.
  ...
  if (type_ids) {
    const idsList = Array.isArray(type_ids) ? type_ids : [type_ids];
    idsList.forEach((id) => {
  ...
  if (collection_ids) {
    const collectionList = Array.isArray(collection_ids)
      ? collection_ids
      : [collection_ids];
    collectionList.forEach((id) => {
...
/**
 * GET /store/{store_id}/products/{slug}?field=slug
